...
export function createTaskRunnerRuntime(options: CreateTaskRunnerRuntimeOptions) {
  ...
  async function writeRunManifest<T extends TaskRunBase>(run: T): Promise<string> {
    return writeRunFile(run.runId, 'run.json', JSON.stringify(run, null, 2));
  }

  async function captureScreenshot<T extends TaskRunBase>(run: T, tabId: string | undefined, label: string): Promise<ScreenshotArtifact | null> {
